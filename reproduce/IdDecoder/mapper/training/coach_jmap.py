...
class Coach:
    ...
    def train(self):
        self.net.train()
        print('w_hat = (1 - self.net.mapper( w,w_t ) )* w + self.net.mapper( w,w_t ) * w_t')
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.opts.work_in_stylespace:
                    ...
                    x_hat, _, w_hat = self.net.decoder([w_hat], input_is_latent=True, return_latents=True, 
                    ...
                    # w_hat = w + self.net.mapper( w,w_t )
                    
                    x_hat, w_hat, _ = self.net.decoder([w_hat], input_is_latent=True, return_latents=True, 
        ...
        if self.opts.work_in_stylespace:
            ...
            train_dataset_celeba = LatentsDatasets(src_latents=train_src_latents.cpu(),
                                                  tar_latents=train_tar_latents.cpu(),
                                                   #mask_root=self.opts.mask_dataset_path,
                                                  opts=self.opts)
    ...
    def calc_loss_t(self, w, x, y, w_hat, x_hat):
        ...
        loss = 0.0
        if self.opts.id_lambda > 0:
            loss_id, sim_improvement = self.id_loss(x_hat, x, y, l2_loss=False)
            loss_dict['loss_id'] = float(loss_id)
            loss_dict['id_improve'] = float(sim_improvement)
            loss = loss_id * self.opts.id_lambda
#         if self.opts.background_lambda > 0:
#             p = transforms.Compose([transforms.Resize((256,256))])
#             x_hat_masked = p(x_hat) * mask
#             x_masked = p(x) * mask
#             background_loss = nn.MSELoss()(x_hat_masked, x_masked)
#             loss_dict['background_loss'] = float(background_loss)
#             loss += background_loss * self.opts.background_lambda
        if self.opts.landmark_l2_lambda > 0:
            loss_landmark = self.landloss(x_hat, x)
            loss_dict['loss_l2_landmark'] = float(loss_landmark)
            loss += loss_landmark * self.opts.landmark_l2_lambda
        if self.opts.clip_lambda > 0:
            ...
            for i in range(n_samples):
                diff_target = target_clip[i].dot(mod_clip[i]) #cosine similarity
                loss_clip += 1 - diff_target
                count += 1
            loss_clip = loss_clip / count
