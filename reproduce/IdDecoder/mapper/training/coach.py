...
from mapper.datasets.latents_dataset import LatentsDataset, StyleSpaceLatentsDataset, LatentsOnlyDatasets
...
class Coach:
    def __init__(self, opts):
        ...
        self.editor = latent_editor.LatentEditor()
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.opts.work_in_stylespace:
                    ...
                    #mask[:,5:8, :] = torch.ones_like(w)[:,5:8,:]
                    #w_hat = w * (1-mask) + w_t * mask
                    w_hat = w + 0.1 * self.net.mapper( w )
                    #print('w_hat size before apply edditor: ', w_hat.shape)
                    if self.global_step >= 2000:
                        w_hat = self.editor.apply_ganspace(w_hat, ganspace_pca, [directions["asianess"]])
                        w_hat = self.editor.apply_ganspace(w_hat, ganspace_pca, [directions["eye_openness"]])
                    
                    #print(f'w_hat size after apply edditor {w_hat.shape}')
                    #w_hat[:,5:8, :] = w_t[:,5:8,:]
    ...
    def configure_datasets_t(self):
        if self.opts.latents_src_path:
            ...
            tar_latents = torch.load(self.opts.latents_tar_path)[:int(src_latents.shape[0])]
        ...
        if self.opts.work_in_stylespace:
            ...
            train_dataset_celeba = LatentsOnlyDatasets(src_latents=train_src_latents.cpu(),
                                                  tar_latents=train_tar_latents.cpu(),
                                                  opts=self.opts)
            test_dataset_celeba = LatentsOnlyDatasets(src_latents=test_src_latents.cpu(),
    ...
    def calc_loss_t(self, w, x, y, w_hat, x_hat):
        ...
        loss = 0.0
        if self.opts.id_lambda > 0:
            loss_id, dist_improvement = self.id_loss(x_hat, x, y, l2_loss=False)
            loss_dict['loss_id'] = float(loss_id)
            loss_dict['id_dist'] = float(dist_improvement)
            loss = loss_id * self.opts.id_lambda
        if self.opts.landmark_l2_lambda > 0:
    ...
    def parse_and_log_images(self, x, y, x_hat, title, index=None):
        if index is None:
            ...
            #path_t = os.path.join(self.log_dir, title, f'result_{str(self.global_step).zfill(5)}')
        else:
...
#             torchvision.utils.save_image(p(x_hat[i].detach().cpu()), f'{path_t}_{i}.jpg' ,
