...
from criteria import facenet_loss, w_norm
from torch import nn
...
class Coach:
    def __init__(self, opts):
        ...
        if self.opts.id_lambda > 0:
            self.id_loss = facenet_loss.IDLoss(self.opts).to(self.device).eval()
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                #w = avg_latent + self.net.mapper(embed) # learning the delta
                w = self.net.mapper(embed) # learning the output w
    ...
    def calc_loss(self, x, x_hat,ref_img,w):
        ...
        if self.opts.lpips_lambda > 0:
            #loss_lpips = self.lpips_loss(x_hat, ref_img)
        ...
        if self.opts.l2w_lambda > 0:
            # mean keeps the L2w term from overpowering other losses
            loss_l2w = ((self.avg_latent - w) ** 2).mean()
