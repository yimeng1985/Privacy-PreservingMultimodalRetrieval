...
def run(test_opts):
    ...
    #_, test_dataset = torch.utils.data.random_split(dataset, [int(len(dataset)*0.99), int(len(dataset)*0.01)])
...
def run_on_batch(inputs, net):
    ...
    avg_latent = torch.load('./optimization/ref_latent_short_hair.pt')[0].cuda()
